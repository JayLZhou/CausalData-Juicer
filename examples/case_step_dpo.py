...
episodes = [
    ...
    if not ep.outcome.success and (not args.tasks or ep.task_id in args.tasks)
]
...
llm = DiskCachedLLM(
    OpenAICompatClient("http://127.0.0.1:8010/v1", "Qwen/Qwen2.5-7B-Instruct", temperature=0.8),
    ...
    for i in range(args.k):
        resp = llm.complete(
            [
                {
                    "role": "system",
                    "content": "Propose the complete next write_file action as one "
        ...
        unit = replayer.paired_replay(
            ep, snapshots, iv, n_repro=2, control_cache=control_cache, early_stop_repro=True
        )
        branches.append((iv.new_action, unit.flipped))
        prm_rows.append(
            {
                "state": state,
                "action": render_action(iv.new_action),
                "label": int(unit.flipped),
                "task_id": ep.task_id,
        ...
        for bad, ok_b in branches:
            if ok_g and not ok_b:
                dpo_pairs.append(
                    {
                        "prompt": state,
                        "chosen": render_action(good),
                        "rejected": render_action(bad),
                        "task_id": ep.task_id,
    ...
    json.dumps(
        {
            ...
            "sampled_branches": len(prm_rows),
            "flipping_branches": sum(r["label"] for r in prm_rows),
            "step_dpo_pairs": len(dpo_pairs),
        },
        indent=2,
    )
)
