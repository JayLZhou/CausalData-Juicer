...
only a tiny pointer file (``.causal_data_juicer_env.json``) that snapshots and
...
ENV_POINTER = ".causal_data_juicer_env.json"
...
class EnvManager:
    ...
    def ensure(self, env: TaskEnv) -> Path:
        ...
        marker = env_dir / ".causal_data_juicer_ready.json"
    ...
    def provenance(self, env: TaskEnv) -> dict:
        marker = self.root / env.key() / ".causal_data_juicer_ready.json"
        return json.loads(marker.read_text()) if marker.exists() else {}


def write_env_pointer(workspace: Path, python: Path) -> None:
    (Path(workspace) / ENV_POINTER).write_text(
        json.dumps({"python": str(python)}) + "\n"
    )


def resolve_python(workspace: Path) -> str:
    """Interpreter for this workspace: its env pointer, else our own."""
    pointer = Path(workspace) / ENV_POINTER
    if pointer.exists():
        return json.loads(pointer.read_text())["python"]
