...
| # | Claim | 实验 | 阈值 / 判死线 | 状态 |
|---|---|---|---|---|
| A1 | 确定性子集上 flip 可复现 | `causeforge demo`（toy workload, n_repro=3） | ≥ 90%，否则停下修 determinism | ✅ **100% (18/18)**, 2026-07-29 |
...
| A9 | live agent 下对照支 digest 匹配率（新指标） | M1.5 bench 全量采集 | 报告数值 + 分解不匹配来源；toy 上界 100% | ⬜ M1.5（埋点已就位） |
