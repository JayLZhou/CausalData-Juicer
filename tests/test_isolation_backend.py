...
from causal_data_juicer.runtime.exec_backend import DEFAULT_LIMITS, describe, probe, wrap
...
def test_network_is_really_unreachable_inside(tmp_path):
    ...
    threading.Thread(target=lambda: srv.accept() if True else None,
                     daemon=True).start()
    try:
        code = (f"import socket; s = socket.socket(); s.settimeout(3); "
                f"raise SystemExit(0 if s.connect_ex(('127.0.0.1', {port})) != 0 else 1)")
        probe_argv = [sys.executable, "-c", code]
        assert subprocess.run(probe_argv).returncode == 1     # reachable outside
...
def test_egress_is_blocked_inside(tmp_path):
    code = ("import socket; s = socket.socket(); s.settimeout(3); "
...
def test_memory_limit_kills_oversized_allocation(tmp_path):
    ...
    argv = wrap([sys.executable, "-c", "x = bytearray(1024**3)"], tmp_path,
                limits={"as_bytes": 256 * 1024**2})
    proc = _run(argv)
    assert proc.returncode != 0
    argv_ok = wrap([sys.executable, "-c", "x = bytearray(10 * 1024**2)"], tmp_path,
...
container_only = pytest.mark.skipif(
    ...
    reason=f"container runtime unavailable here: {CAPS.detail}")
...
def test_container_memory_limit_enforced(tmp_path):
    proc = _run(wrap(["python", "-c", "x = bytearray(2 * 1024**3)"], tmp_path,
                     limits={"as_bytes": 256 * 1024**2}))
    assert proc.returncode != 0
...
def test_container_workspace_is_writable_and_syncs_back(tmp_path):
    ws = tmp_path / "ws"
    ws.mkdir()
    proc = _run(wrap(["python", "-c",
                      "open('made-inside.txt', 'w').write('from container')"], ws))
    assert proc.returncode == 0, proc.stderr
