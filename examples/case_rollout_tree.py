...
parser.add_argument(
    "--tasks", nargs="*", default=["p02_str_coercion", "k01_info", "s03_engine_execute"]
)
...
episodes = [
    ep for ep in store.load_episodes() if not ep.outcome.success and ep.task_id in args.tasks
]
...
llm = DiskCachedLLM(
    OpenAICompatClient(args.base_url, "Qwen/Qwen2.5-7B-Instruct", temperature=0.9),
    ...
    json.dumps(
        {
            ...
            "nonzero_advantages": sum(1 for f in flat if f.get("advantage")),
        },
        indent=2,
    )
)
