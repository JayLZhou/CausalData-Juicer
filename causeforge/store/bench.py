...
from causeforge.store.dag import dag_stats, select_checkpoints, storage_bytes
...
def print_bench(report: dict) -> None:
    d = report["dag"]
    print(f"trace DAG: {d['snapshot_references']} snapshot refs -> {d['unique_trees']} unique "
          f"trees (sharing {d['sharing_ratio']}x, bytes saved {d['bytes_saved_pct']}%)")
    print(f"{'policy':<12}{'ckpts':<7}{'storage':<10}{'fork_s':<9}{'prefix':<8}"
          f"{'speedup':<9}{'equiv'}")
    for r in report["policies"]:
        print(f"{r['policy']:<12}{r['checkpoints_kept']:<7}"
