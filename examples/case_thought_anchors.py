...
PROBLEM = (
    "A workshop has 3 machines. Each machine produces 14 parts per hour. "
...
llm_hot = DiskCachedLLM(
    OpenAICompatClient("http://127.0.0.1:8021/v1", "Qwen/Qwen2.5-7B-Instruct", temperature=0.9),
    OUT / "llm_cache",
)
...
class Stepify:
    ...
    def next_action(self, task_id, idx, history):
        ...
        if nxt is None:
            return None
        action, llm = nxt
        if action.tool == "write_file" and action.args.get("path") == "thoughts.md":
...
direction = (
    "stress (which thought, changed, breaks it)"
    if baseline_ok
    else "repair (which thought, changed, fixes it)"
)
thought_steps = [s for s in episode.steps if s.action.args.get("path") == "thoughts.md"]
...
for step in thought_steps:
    ...
    for v in range(K_RESAMPLES):
        ...
        unit = replayer.paired_replay(
            episode, snapshots, swap, n_repro=1, continuation_policy=thinker
        )
    ...
    rows.append(
        {
            "step": step.index,
            "thought": original_thought[:90],
            "resamples": K_RESAMPLES,
            "flips": survived,
