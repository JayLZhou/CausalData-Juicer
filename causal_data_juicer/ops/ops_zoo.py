...
class FixerLLM(SourceOp):
    ...
    def run(self, ctx: OpContext) -> OpContext:
        ...
        ctx.sources.append(
            FixerLLMSource(
                llm,
                candidates_per_failure=int(self.params.get("candidates", 2)),
                ledger=ctx.ledger,
                tests_by_task=self.params.get("tests_by_task"),
            )
        )
        return ctx
...
class Resample(SourceOp):
    ...
    def run(self, ctx: OpContext) -> OpContext:
        ...
        ctx.sources.append(ResampleSource(llm, k=int(self.params.get("k", 3)), ledger=ctx.ledger))
        return ctx
    ...
    def run(self, ctx: OpContext) -> OpContext:
        ...
        for i, unit in enumerate(ctx.units):
            if unit.tier >= EvidenceTier.REPRODUCIBLE:
                ctx.units[i] = minimize_unit(
                    services["replayer"], eps[unit.episode_id], ctx.snapshots, unit
                )
        return ctx
...
class ExportViews(CompileOp):
    ...
    def run(self, ctx: OpContext) -> OpContext:
        ...
        ctx.exports.update(
            {
                k: str(v)
                for k, v in compile_all(ctx.units, ctx.episodes, ctx.workdir / "exports").items()
            }
        )
        return ctx
...
class ExportTRL(CompileOp):
    ...
    def run(self, ctx: OpContext) -> OpContext:
        ...
        for fmt in self.params.get("formats", ["trl-sft", "trl-dpo", "verl"]):
            ...
            path = ADAPTERS[fmt](
                ctx.units, ctx.episodes, ctx.workdir / "exports" / f"{fmt}{suffix}"
            )
