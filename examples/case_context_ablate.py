        ...
        if idx == 0:
            return ToolCall(tool="write_file",
                            args={"path": "context.md",
                                  "content": context_md(self.docs)}), None
...
for name in TASK["docs"]:
    ...
    drop = Intervention(
        ...
        new_action=ToolCall(tool="write_file",
                            args={"path": "context.md", "content": context_md(reduced)}),
        source=f"ablate:{name}")
    unit = replayer.paired_replay(episode, snapshots, drop, n_repro=1,
                                  continuation_policy=reader)
    survived = bool(unit.intervened_outcome and unit.intervened_outcome.success)
    rows.append({"doc": name, "answer_survived_without_it": survived,
...
print(json.dumps({"docs": len(rows),
                  "supports_found": [r["doc"] for r in rows if r["causal_support"]],
                  "attribution_accuracy": f"{correct}/{len(rows)}"}, indent=2))
