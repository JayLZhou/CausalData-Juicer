...
def _init_worker(blobs_root: str, scratch_root: str, verify_argv: list[str] | None):
    ...
    _WORKER["replayer"] = Replayer(
        default_registry(), UnsafeLocalWorkspace(BlobStore(Path(blobs_root)), scratch), verifier
    )
...
def _validate_one(payload: tuple[str, str, str, int]) -> str:
    ...
    unit = _WORKER["replayer"].paired_replay(
        ...
        intervention,
        n_repro=n_repro,
        control_cache=_WORKER["control_cache"],
...
def validate_parallel(
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(str(blobs_root), str(scratch_root), verify_argv),
    ) as pool:
        return [CausalUnit.model_validate_json(r) for r in pool.map(_validate_one, payloads)]
