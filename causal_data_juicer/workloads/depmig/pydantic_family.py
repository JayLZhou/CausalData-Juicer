...
def build_tasks() -> list[DepMigTask]:
    ...
    tasks.append(
        DepMigTask(
            ...
            migration_points=[
                "BaseSettings import (moved to pydantic-settings; migrate to BaseModel)"
            ],
            files={
                "config.py": (
    ...
    tasks.append(
        DepMigTask(
            ...
            migration_points=[
                "conlist(min_items=/max_items=) removed (-> min_length=/max_length=, two models)"
            ],
            files={
                "batches.py": (
                    "from typing import List\n"
    ...
    tasks.append(
        DepMigTask(
            id="p09_settings_optionals",
            family=FAMILY,
            tier=2,
            description="A service config stack combining BaseSettings and Optional-default fields.",
            migration_points=[
                "BaseSettings import moved",
                "Optional[T] needs explicit defaults (two files)",
            ],
            files={
                "conf.py": (
                    "from typing import Optional\n"
