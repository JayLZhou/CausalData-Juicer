    ...
    policy = ScriptedPolicy(
        [
            ScriptedStep(
                action=ToolCall(
                    tool="write_file",
                    args={"path": "solution.sql", "content": "\n".join(sql_lines) + "\n"},
                )
            ),
    ...
    return Intervention(
        ...
        edits=[
            ArgEdit(
                arg="content",
                op="patch_lines",
                patches=[LinePatch(line=ln, text=txt) for ln, txt in patches],
            )
        ],
    )
...
unit = replayer.paired_replay(
    episode, snapshots, clause_edit(list(REPAIR["clause_fixes"].items())), n_repro=3
)
    ...
    rows.append(
        {
            ...
            "clause_line": line,
            "clause": text,
            "in_minimal_causal_set": line in minimal_lines,
...
for line, alternatives in STRESS["perturbations"].items():
    for alt in alternatives:
        outcome = replayer.intervened_flip(
            episode_s, snapshots_s, clause_edit([(line, alt)]), ledger
        )
        rows.append(
            {
                "task_id": STRESS["id"],
...
summary = {
    ...
    "stress": {
        "perturbations": sum(len(v) for v in STRESS["perturbations"].values()),
